...
# alef:hash:d9c05879d22424e047134d54d3619fdc31b63319f70936ef9cfc55334cf7991e
...
defmodule Kreuzberg do
  ...
  @spec extract_bytes_async(binary(), String.t(), keyword()) ::
          {:ok, map()} | {:error, atom, String.t()}
  def extract_bytes_async(content, mime_type, opts \\ []) do
    config = Keyword.get(opts, :config)
    config_json = serialize_config(config)
    Kreuzberg.Native.extract_bytes_async(
      content,
      mime_type,
      config_json
    )
  ...
  @spec extract_bytes_sync(binary(), String.t(), keyword()) ::
          {:ok, map()} | {:error, atom, String.t()}
  def extract_bytes_sync(content, mime_type, opts \\ []) do
    config = Keyword.get(opts, :config)
    config_json = serialize_config(config)
    Kreuzberg.Native.extract_bytes_sync(
      content,
      mime_type,
      config_json
    )
  ...
  @spec embed_texts_async([String.t()], keyword()) ::
          {:ok, [[float()]]} | {:error, atom, String.t()}
  def embed_texts_async(texts, opts \\ []) do
    config = Keyword.get(opts, :config)
    config_json = serialize_config(config)
    Kreuzberg.Native.embed_texts_async(
      texts,
      config_json
    )
  end
  ...
  @spec embed_texts([String.t()], keyword()) :: {:ok, [[float()]]} | {:error, atom, String.t()}
  def embed_texts(texts, opts \\ []) do
    config = Keyword.get(opts, :config)
    config_json = serialize_config(config)
    Kreuzberg.Native.embed_texts(
      texts,
      config_json
    )
  end
  ...
  defp serialize_config(config) when is_binary(config), do: config
  defp serialize_config(config) when is_struct(config) do
    config
    |> Map.from_struct()
    |> Enum.reject(fn {_k, v} -> v == nil or _k == :cancel_token end)
    |> Enum.into(%{})
    |> Jason.encode!()
  end
  defp serialize_config(config) when is_map(config) do
    config |> Map.drop(["cancel_token", :cancel_token]) |> Jason.encode!()
  end
end
