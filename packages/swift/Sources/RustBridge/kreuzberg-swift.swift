...
public func drawingTypeFromJson<GenericIntoRustString: IntoRustString>(_ json: GenericIntoRustString) throws -> DrawingType {
    try { let val = __swift_bridge__$drawing_type_from_json({ let rustString = json.intoRustString(); rustString.isOwned = false; return rustString.ptr }()); if val.is_ok { return DrawingType(ptr: val.ok_or_err!) } else { throw RustString(ptr: val.ok_or_err!) } }()
}
